...
assert(jobs.keys == %w[verify release], "workflow must contain verify and release jobs")
verify = jobs.fetch("verify")
release = jobs.fetch("release")

assert(verify["runs-on"] == "macos-15", "verify job must run on macos-15")
...
assert(release["runs-on"] == "macos-15", "release job must run on macos-15")
...
assert(positions.none?(&:nil?) && positions == positions.sort, "protected release steps are missing or out of order")
...
token = steps.fetch(names.index("Create tap GitHub App token"))
...
notify = steps.fetch(names.index("Notify Homebrew tap"))
assert(notify.fetch("env") == { "TAP_GH_TOKEN" => "${{ steps.tap-token.outputs.token }}" }, "tap notification may receive only the installation token")
assert(notify.fetch("run") == 'scripts/dispatch-homebrew-update.sh "$RELEASE_TAG"', "tap notification must dispatch the published tag")
assert(!notify.key?("continue-on-error"), "tap notification failures must fail the workflow")

all_uses_steps = (verify_steps + steps).select { |step| step.key?("uses") }
assert(all_uses_steps.length == 4, "only checkout, setup-node, release checkout, and GitHub App token actions are allowed")
...
assert(workflow_source.scan(/\$\{\{\s*secrets\.[A-Z0-9_]+\s*\}\}/).all? { |reference| release.to_s.include?(reference) }, "all secret references must be isolated to release")
...
COMMIT_CHECK_HARNESS="$TEMP_ROOT/verify-release-commit.sh"
mkdir -p "$FAKE_BIN"
...
extract_workflow_step release "Verify release commit" "$COMMIT_CHECK_HARNESS"
...
# The protected job must accept only the exact verified commit and fail closed otherwise.
(
    cd -- "$TAG_REPOSITORY"
...
status=$?
set -e
[[ "$status" -ne 0 ]] || fail "protected release accepted a mismatched verified commit"
set +e
output="$(cd -- "$TAG_REPOSITORY" && VERIFIED_RELEASE_COMMIT='refs/tags/v1.2' /bin/bash "$COMMIT_CHECK_HARNESS" 2>&1)"
status=$?
set -e
[[ "$status" -ne 0 ]] || fail "protected release accepted non-commit provenance"
