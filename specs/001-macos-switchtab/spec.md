...
**Status**: Current implementation scope as of 2026-08-02
...
> Current repository note (2026-08-02): the checked-in app target implements
> independently configurable current-app window and application switchers.
> Fresh installs enable both modes with Cmd+` and Cmd+Tab respectively. Existing
> installs preserve an explicit application-switching choice and otherwise keep
> application switching disabled during legacy migration. Native macOS Cmd+Tab
> remains available while application switching is disabled, when EventTap
> registration is unavailable, and after SwitchTab exits.
...
### User Story 3 - Configure Application Switching (Priority: P1)

As a macOS user, I want to configure or disable the SwitchTab application
switcher so I can use one keyboard-first overlay for application changes while
retaining native Cmd+Tab behavior whenever that mode is disabled.

**Why this priority**: Application switching must remain independently
controllable and must not change the current-app window flow or macOS behavior
...
1. **Given** application switching is disabled, **When** the user presses
   Cmd+Tab, **Then**
   macOS shows its native application switcher and no SwitchTab application
   overlay appears.
2. **Given** application switching is enabled and Accessibility is granted,
...
5. **Given** Accessibility is missing, **When** application switching is
   enabled, **Then** the setting remains saved, the EventTap is not active, its
   configured shortcut is not consumed, and Settings shows application-specific
   recovery guidance.
...
- Application switching is enabled without Accessibility permission.
- The application EventTap fails to register or only partially registers.
- The application-switching toggle changes while its overlay is visible.
- SwitchTab exits or crashes while application switching is enabled.
...
- **FR-013**: The switcher overlay MUST default to shortcut-release
  ...
  available.
- **FR-014**: Settings MUST focus on independent current-app window and
  application-switching status, toggle, shortcut, and reset controls, app
  preferences, update controls when available, and permission status, without
  arbitrary switcher mode or selection-behavior controls.
- **FR-015**: Users MUST be able to focus the selected window by clicking its
  visible row, icon, or preview.
- **FR-016**: Settings MUST expose an independently persisted `Application
  Switching` enabled state and configurable shortcut. Fresh installs MUST
  default it to enabled with Cmd+Tab; legacy migration MUST preserve an explicit
...
- **FR-018**: When application switching is disabled, EventTap registration
  ...
  for its configured shortcut. A failed or partial registration MUST NOT
  consume that shortcut.
...
- **FR-021**: Accessibility guidance MUST identify the EventTap requirement for
  application switching while preserving the existing Accessibility and
...
- **Permission State**: Current ability to observe and focus windows, including
  ...
  application-switching EventTap can intercept its configured shortcut; Screen Recording
...
- **SC-006**: With application switching disabled, unavailable, or after SwitchTab exits,
...
- Fresh installs enable application switching with Cmd+Tab. Existing installs
  preserve an explicit prior application-switching state and otherwise migrate
  with it disabled so an update does not unexpectedly replace native Cmd+Tab.
