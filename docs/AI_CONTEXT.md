...
- Thumbnail work is demand-driven and bounded: selected items precede visible items, capture stays serial, pending work is capped, and cached encoded/decoded image cost is evicted by LRU. Dismissal and generation changes discard stale work while retaining completed entries; warning pressure trims the cache and critical pressure clears it.
- Thumbnail cache reuse lasts for the SwitchTab process lifetime and is bounded to 16 entries or 24 MiB (warning trim: 8 entries or 12 MiB). A retained thumbnail is shown immediately on a later presentation and refreshed once per generation. Normal dismissal preserves completed entries; Screen Recording permission loss, confirmed window destruction, owner-process termination, SwitchTab termination, and critical pressure clear the affected cache entries.
