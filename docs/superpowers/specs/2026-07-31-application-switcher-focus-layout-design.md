...
- 긴 앱 이름은 그리드 배치에 영향을 주지 않으면서 아이콘 폭보다 넓게
  최대 240pt까지 표시할 수 있다. 한 개 앱일 때는 패널이 이 폭을 확보하고,
  여러 열의 가장자리 캡션은 그리드 안쪽으로 펼친다. 한 줄을 유지하며 패널
  경계 안에서도 담을 수 없을 때만 꼬리 말줄임표를 사용한다.
...
- 최대 240pt 캡션이 패널 경계 안에서 꼬리 말줄임으로 끝난다.
