...
For close-selection stability, keep `SwitcherSession` as the authority for neighbor selection and continue removing confirmed windows by stable window ID. Render the grid with each item's stable ID as the `ForEach` identity instead of its transient array index. After a confirmed removal, update layout and advance the existing keyboard scroll token so the corrected selection is visible. If selection moved while destruction confirmation was pending, the session's identity-preserving index adjustment remains authoritative.
...
5. The presentation model advances its scroll token, re-renders stable-ID tiles, and scrolls the corrected selection into view.
...
- Add controller tests for delayed destruction after additional navigation, repeated close-and-cycle commands, stable selected identity, and scroll-token advancement after confirmed removal.
